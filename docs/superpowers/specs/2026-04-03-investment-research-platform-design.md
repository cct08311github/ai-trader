...
```
                    ...
                    │  /api/reports/*        (Phase 1D)     │  ← Note: actual implementation uses /api/research-reports/* (see research_reports.py)
                    │  /api/macro/*          (Phase 2)      │
                    │  /api/sector/*         (Phase 2)      │
                    │  /api/stream/*         (SSE)          │
                    └──────────────┬───────────────────────┘
...
| `frontend/backend/app/api/research_reports.py` | GET /api/research-reports/list, /api/research-reports/{id}, POST /api/research-reports/generate (Note: spec shows `/api/reports/*` but actual router prefix is `/api/research-reports/*`) |
